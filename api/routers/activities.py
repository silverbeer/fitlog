...
async def get_status(
    days: int = Query(7, description="Number of days to show in status"),
):
    """
...
async def get_report(
    days: int = Query(
        7, description="Number of days to report on (7=week, 30=month, 365=year)"
    ),
):
    """
    ...
    try:
        ...
        return {
            ...
            "personal_records": {
                "fastest_run": {"distance": 0.0, "pace": "00:00:00", "date": None},
                "longest_run": {"distance": 0.0, "date": None},
                "highest_cadence": {"cadence": 0, "date": None},
            },
            "pushups_summary": {
                "total_sessions": 0,
                "total_pushups": 0,
...
async def import_from_smashrun(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to import from Smashrun: {str(e)}"
        )
